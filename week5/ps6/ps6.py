...
class PlaintextMessage(Message):
    ...
    def get_encrypting_dict(self):
        ...
        copydict=deepcopy(self.encrypting_dict)
        return copydict
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(1,26):
            count=0
            processed_text = deepcopy(self.apply_shift(i))
            wl=processed_text.split(' ')
            for word in wl:
                if word in self.valid_words:
                    count=count+1
            shift_value_store.update({i:count})
        
        best_shift_value=(max(shift_value_store, key=shift_value_store.get))
        result=(best_shift_value, self.apply_shift(best_shift_value))
